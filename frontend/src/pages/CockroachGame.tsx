...
const SEED_COMMENTS = [
  "Таракандын анатомиясы абдан так иштелип чыккан 🪳",
  "Биология сабагы үчүн сонун интерактивдүү курал!",
];
...
const CockroachGame: FC = () => {
  ...
  const [likes, setLikes] = useState(() => loadJson("cockroach_likes", 74));
  const [isLiked, setIsLiked] = useState(() =>
    loadJson("cockroach_is_liked", false),
  );
  const [shares, setShares] = useState(() => loadJson("cockroach_shares", 32));
  const [comments, setComments] = useState<string[]>(() =>
    loadJson("cockroach_comments", SEED_COMMENTS),
  );
  ...
  useEffect(() => {
    localStorage.setItem("cockroach_likes", JSON.stringify(likes));
  }, [likes]);

  useEffect(() => {
    localStorage.setItem("cockroach_is_liked", JSON.stringify(isLiked));
  }, [isLiked]);

  useEffect(() => {
    localStorage.setItem("cockroach_shares", JSON.stringify(shares));
  }, [shares]);

  useEffect(() => {
    localStorage.setItem("cockroach_comments", JSON.stringify(comments));
  }, [comments]);
  ...
  const shareToSocial = (
    ...
    const text = encodeURIComponent(
      `Биологиянын жандуу таракан оюнун ойноп жатам: ${window.location.href}`,
    );
  ...
  return (
    <div className="relative flex min-h-screen select-none flex-col justify-between overflow-hidden bg-[#0d2111] p-4 font-sans text-white">
      ...
      >
        <div className="relative h-28 w-16 rounded-b-[50px] rounded-t-[30px] border-4 border-black bg-gradient-to-b from-amber-950 via-amber-900 to-amber-950 shadow-[0_10px_0px_#291300]">
          <div className="absolute -top-4 left-1/2 flex h-6 w-8 -translate-x-1/2 items-center justify-around rounded-full border-2 border-black bg-amber-900 px-1">
            <div className="h-2 w-2 rounded-full border border-amber-600 bg-black" />
            <div className="h-2 w-2 rounded-full border border-amber-600 bg-black" />
          </div>

          <div className="absolute -top-10 left-2 h-6 w-8 origin-bottom-right rotate-[25deg] rounded-tl-full border-l-4 border-t-4 border-black" />
          <div className="absolute -top-10 right-2 h-6 w-8 origin-bottom-left -rotate-[25deg] rounded-tr-full border-r-4 border-t-4 border-black" />

          <div className="absolute bottom-2 left-1/2 top-6 w-0.5 bg-black/60" />

          <div className="absolute -left-4 top-8 h-2.5 w-6 rotate-[15deg] animate-pulse rounded-l-full border border-black bg-gradient-to-r from-black to-amber-900" />
          <div className="absolute -right-4 top-8 h-2.5 w-6 -rotate-[15deg] animate-pulse rounded-r-full border border-black bg-gradient-to-l from-black to-amber-900" />

          <div className="absolute -left-5 top-16 h-2.5 w-7 -rotate-[15deg] animate-pulse rounded-l-full border border-black bg-gradient-to-r from-black to-amber-900" />
          <div className="absolute -right-5 top-16 h-2.5 w-7 rotate-[15deg] animate-pulse rounded-r-full border border-black bg-gradient-to-l from-black to-amber-900" />

          <div className="absolute -left-4 top-24 h-2.5 w-6 rotate-[30deg] animate-pulse rounded-l-full border border-black bg-gradient-to-r from-black to-amber-900" />
          <div className="absolute -right-4 top-24 h-2.5 w-6 -rotate-[30deg] animate-pulse rounded-r-full border border-black bg-gradient-to-l from-black to-amber-900" />
        </div>

        <span className="mt-3 whitespace-nowrap rounded-md border border-amber-600 bg-black/80 px-2.5 py-1 text-[10px] font-black tracking-wider text-amber-200 shadow-[2px_2px_0px_#000]">
          🪳 Жандуу таракан (Анатомия)
        </span>
      </div>
      ...
      <div className="z-30 mx-auto my-auto grid w-full max-w-5xl grid-cols-1 items-center gap-8 p-4 md:grid-cols-2">
        <div className="rounded-2xl border-4 border-black bg-[#1b4226] p-6 shadow-[6px_6px_0px_#000]">
          <h2 className="mb-4 text-sm font-black uppercase tracking-wider text-green-300">
            Таракандар жөнүндө маалымат:
          </h2>
          <p className="mb-4 text-xs font-medium leading-relaxed text-slate-200">
            Таракандар — жер бетиндеги эң байыркы жана өтө чыдамдуу
            курт-кумурскалардын бири. Алардын омурткасыз түзүлүшү, фасеткалык
            көздөрү жана үч жуп буттан турган анатомиясы энтомология илиминде
            терең изилденет. Бул интерактивдүү модель алардын жандуу
            жүрүм-турумун жана түзүлүшүн көрсөтөт.
          </p>
        ...
        <div className="rounded-2xl border-4 border-black bg-white p-6 text-center text-black shadow-[8px_8px_0px_#000]">
          <span className="mb-4 block animate-bounce text-6xl">🪳</span>
          <h3 className="mb-2 text-base font-black uppercase">
            Интерактивдүү анатомия
          </h3>
          <p className="mb-6 text-xs font-bold text-slate-600">
            Чычканды экрандын каалаган жерине жылдырып, таракандын артыңыздан
            кандай жылмакай ээрчип келе жатканын жана буттары кантип кыймылдаарын
            байкаңыз.
          </p>
          <div className="rounded-xl border-2 border-black bg-green-200 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-green-900">
            РЕЖИМ АКТИВДҮҮ
          </div>
        </div>
      </div>
