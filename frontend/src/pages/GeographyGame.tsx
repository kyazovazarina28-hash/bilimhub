...
import { geographyGameLevels } from "../data/geographyGameLevels";
...
const SEED_COMMENTS: Record<number, GeoComment[]> = {
  1: [
    { id: 1, user: "Айсулуу", text: "Дүйнө картасын үйрөнүү абдан сонун! 🗺️" },
    { id: 2, user: "Данияр", text: "Евразия туура табылды! ⛰️" },
  ],
  2: [
    { id: 1, user: "Айсулуу", text: "Дүйнө картасын үйрөнүү абдан сонун! 🗺️" },
    { id: 2, user: "Данияр", text: "Кийинки суроолорго Кыргызстанды кошкулачы 🇰🇬" },
  ],
};

const DEFAULT_LIKES: Record<number, number> = { 1: 87, 2: 72 };
const DEFAULT_SHARES: Record<number, number> = { 1: 18, 2: 14 };
...
const defaultLikes = (id: number) => DEFAULT_LIKES[id] ?? 22 + ((id * 5) % 40);
const defaultShares = (id: number) => DEFAULT_SHARES[id] ?? 6 + (id % 14);

const GeographyGame: FC = () => {
  ...
  const [likes, setLikes] = useState<Record<number, number>>(() =>
    loadJson("geo_likes", DEFAULT_LIKES),
  );
  const [isLiked, setIsLiked] = useState<Record<number, boolean>>(() =>
    loadJson("geo_is_liked", {}),
  );
  const [shares, setShares] = useState<Record<number, number>>(() =>
    loadJson("geo_shares", DEFAULT_SHARES),
  );
  const [comments, setComments] = useState<Record<number, GeoComment[]>>(() =>
    loadJson("geo_comments", SEED_COMMENTS),
  );
  ...
  const currentQ = geographyGameLevels[currentStage] ?? geographyGameLevels[0];
  ...
  useEffect(() => {
    localStorage.setItem("geo_likes", JSON.stringify(likes));
  }, [likes]);

  useEffect(() => {
    localStorage.setItem("geo_is_liked", JSON.stringify(isLiked));
  }, [isLiked]);

  useEffect(() => {
    localStorage.setItem("geo_shares", JSON.stringify(shares));
  }, [shares]);

  useEffect(() => {
    localStorage.setItem("geo_comments", JSON.stringify(comments));
  }, [comments]);
  ...
  const getRank = () => {
    if (score.points >= 600) return "🌍 ДҮЙНӨ САЯКАТЧЫСЫ";
    if (score.points >= 300) return "🗺️ КАРТАГРАФ";
    if (score.points >= 100) return "🧭 САЯКАТЧЫ";
  ...
  const handleNext = () => {
    if (selectedOption === null) return;
    if (currentStage < geographyGameLevels.length - 1) {
  ...
  const shareToSocial = (
    ...
    const text = encodeURIComponent(
      `Географиялык саякат оюнун ойноп жатам! 🌍 ${window.location.href}`,
    );
  ...
  if (isFinished) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#0a2e1a] p-4 font-sans text-white">
        <div className="w-full max-w-md rounded-2xl border-4 border-black bg-[#fffbeb] p-8 text-center text-black shadow-[8px_8px_0px_#000]">
          ...
          <div className="mb-6 rounded-xl border-4 border-black bg-black p-4 text-white">
            <p className="mb-2 text-sm font-black text-[#a3e635]">{getRank()}</p>
          ...
          <div className="space-y-3">
            ...
            <button
              type="button"
              onClick={() => navigate("/subjects/geografiya")}
  ...
  return (
    <div className="relative flex min-h-screen flex-col justify-between overflow-hidden bg-[#0a2e1a] p-4 font-sans text-white">
      <div
        ref={trailRef}
      ...
      <div className="z-10 mx-auto mt-2 flex w-full max-w-7xl items-center justify-between">
        ...
        <div className="text-center">
          <h1 className="text-xl font-black tracking-wider text-[#a3e635] md:text-2xl">
            ГЕОГРАФИЯЛЫК САЯКАТ 🗺️
          </h1>
          <p className="mt-1 text-xs font-bold uppercase tracking-widest text-emerald-300">
            Деңгээл: {currentStage + 1} / {geographyGameLevels.length}
          </p>
        </div>
      ...
      <div className="z-10 mx-auto my-auto flex w-full max-w-7xl flex-col items-center justify-center gap-8 px-4 lg:flex-row">
        <div className="flex w-full max-w-lg flex-col items-center lg:w-1/2">
          <div className="relative mb-8 w-full rounded-2xl border-4 border-black bg-white p-6 text-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <span className="mb-2 block text-xs font-black tracking-wider text-emerald-700">
              {currentQ.character}
            </span>
          ...
          <div className="flex h-56 w-56 animate-pulse items-center justify-center rounded-full border-4 border-black bg-[#134e2a] text-7xl shadow-[0px_0px_40px_rgba(34,197,94,0.35)]">
            {currentQ.icon}
          </div>
        ...
        <div className="flex min-h-[450px] w-full max-w-xl flex-col justify-between rounded-2xl border-4 border-black bg-[#fffbeb] p-6 text-black shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] lg:w-1/2">
          <div>
            <div className="mb-4 inline-block rounded-md border-2 border-black bg-emerald-200 px-3 py-1 text-xs font-black uppercase tracking-wider text-emerald-800">
              {currentQ.tag}
            </div>
          ...
          <div className="mt-6 flex justify-end border-t-2 border-dashed border-gray-300 pt-4">
            <button
              ...
              className={`flex items-center gap-2 rounded-xl border-2 border-black px-6 py-3 font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all ${
                selectedOption !== null
                  ? "bg-[#22c55e] text-black hover:translate-y-0.5"
                  : "cursor-not-allowed bg-gray-300 text-gray-500 shadow-none"
              }`}
            >
              {currentStage === geographyGameLevels.length - 1
                ? "ЖЫЙЫНТЫКТЫ КӨРҮҮ 🏆"
                : "КИЙИНКИ СУРОО ➡️"}
            </button>
          </div>
          ...
          <div className="w-full max-w-md rounded-2xl border-4 border-black bg-[#fffbeb] p-6 text-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            <div className="mb-4 max-h-48 space-y-2 overflow-y-auto">
              {levelComments.map((c) => (
                ...
                >
                  <span className="block font-bold text-emerald-700">{c.user}:</span>
            ...
            <form onSubmit={handleAddComment} className="flex gap-2">
              <input
                ...
                placeholder="География боюнча пикир..."
                className="w-full rounded-xl border-2 border-black p-2 text-sm"
              />
              <button
                type="submit"
                className="rounded-xl border-2 border-black bg-[#22c55e] px-4 text-xs font-black text-black"
              >
                ЖӨНӨТҮҮ
              </button>
            </form>
      ...
      {isShareOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border-4 border-black bg-[#fffbeb] p-6 text-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <div className="mb-4 flex items-center justify-between border-b-2 border-black pb-2">
              <h3 className="text-lg font-black text-emerald-700">🔁 БӨЛҮШҮҮ</h3>
              <button
                type="button"
      ...
      <div className="z-10 mx-auto mt-4 flex w-full max-w-7xl flex-col items-center justify-between gap-2 rounded-xl border-t-4 border-black bg-[#052e16] p-4 sm:flex-row">
