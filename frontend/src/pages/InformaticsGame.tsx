...
import { informaticsGameLevels } from "../data/informaticsGameLevels";

interface InformaticsComment {
  id: number;
  user: string;
  text: string;
}

const SEED_COMMENTS: Record<number, InformaticsComment[]> = {
  1: [
    { id: 1, user: "Айбек", text: "Роботтун суроолору абдан кызыктуу экен! 🤖" },
    { id: 2, user: "Салима", text: "Экилик системаны азыр эле мектептен өткөнбүз 🖥️" },
  ],
  2: [
    { id: 1, user: "Айбек", text: "Роботтун суроолору абдан кызыктуу экен! 🤖" },
    { id: 2, user: "Салима", text: "Алгоритм туура табылды! ⚡" },
  ],
};

const DEFAULT_LIKES: Record<number, number> = { 1: 312, 2: 278 };
const DEFAULT_SHARES: Record<number, number> = { 1: 35, 2: 29 };
...
const defaultLikes = (id: number) => DEFAULT_LIKES[id] ?? 45 + ((id * 6) % 70);
const defaultShares = (id: number) => DEFAULT_SHARES[id] ?? 10 + (id % 20);

const InformaticsGame: FC = () => {
  ...
  const [likes, setLikes] = useState<Record<number, number>>(() =>
    loadJson("informatics_likes", DEFAULT_LIKES),
  );
  const [isLiked, setIsLiked] = useState<Record<number, boolean>>(() =>
    loadJson("informatics_is_liked", {}),
  );
  const [shares, setShares] = useState<Record<number, number>>(() =>
    loadJson("informatics_shares", DEFAULT_SHARES),
  );
  const [comments, setComments] = useState<Record<number, InformaticsComment[]>>(() =>
    loadJson("informatics_comments", SEED_COMMENTS),
  );
  ...
  const currentQ = informaticsGameLevels[currentStage] ?? informaticsGameLevels[0];
  ...
  useEffect(() => {
    localStorage.setItem("informatics_likes", JSON.stringify(likes));
  }, [likes]);

  useEffect(() => {
    localStorage.setItem("informatics_is_liked", JSON.stringify(isLiked));
  }, [isLiked]);

  useEffect(() => {
    localStorage.setItem("informatics_shares", JSON.stringify(shares));
  }, [shares]);

  useEffect(() => {
    localStorage.setItem("informatics_comments", JSON.stringify(comments));
  }, [comments]);
  ...
  const getRank = () => {
    if (score.points >= 600) return "👑 CODE MASTER";
    if (score.points >= 300) return "🤖 ПРОГРАММИСТ";
    if (score.points >= 100) return "💻 КИБЕР САЯКАТЧЫ";
  ...
  const handleNext = () => {
    if (selectedOption === null) return;
    if (currentStage < informaticsGameLevels.length - 1) {
  ...
  const shareToSocial = (
    ...
    const text = encodeURIComponent(
      `Кибер дүйнө оюнун ойноп жатам! 💻 ${window.location.href}`,
    );
  ...
  if (isFinished) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#0a0f1d] p-4 font-sans text-white">
        <div className="w-full max-w-md rounded-2xl border-4 border-black bg-[#fffbeb] p-8 text-center text-black shadow-[8px_8px_0px_#000]">
          ...
          <div className="mb-6 rounded-xl border-4 border-black bg-black p-4 text-white">
            <p className="mb-2 text-sm font-black text-[#22c55e]">{getRank()}</p>
            <div className="mt-3 grid grid-cols-2 gap-3 text-center text-xs">
              ...
              <div>
                ...
                <p className="text-base font-black text-purple-400">{score.crystals}</p>
              </div>
              <div>
          ...
          <div className="space-y-3">
            ...
            <button
              type="button"
              onClick={() => navigate("/subjects/informatika")}
  ...
  return (
    <div className="relative flex min-h-screen flex-col justify-between overflow-hidden bg-[#0a0f1d] p-4 font-sans text-white">
      <div
        ref={trailRef}
      ...
      {shareSuccess && (
        <div className="fixed left-1/2 top-6 z-50 -translate-x-1/2 animate-bounce rounded-xl border-4 border-black bg-[#22c55e] px-6 py-3 text-xs font-black uppercase tracking-wider text-black shadow-[4px_4px_0px_#000]">
      ...
      <div className="z-10 mx-auto mt-2 flex w-full max-w-7xl items-center justify-between">
        ...
        <div className="text-center">
          <h1 className="text-xl font-black tracking-wider text-[#22c55e] md:text-2xl">
            INFORMATICS QUEST ⚡
          </h1>
          <p className="mt-1 text-xs font-bold uppercase tracking-widest text-yellow-400">
            КАТМАР: {currentStage + 1} / {informaticsGameLevels.length}
          </p>
        </div>
      ...
      <div className="z-10 mx-auto my-auto flex w-full max-w-7xl flex-col items-center justify-center gap-8 px-4 lg:flex-row">
        <div className="flex w-full max-w-lg flex-col items-center lg:w-1/2">
          <div className="relative mb-8 w-full rounded-2xl border-4 border-black bg-white p-6 text-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <span className="mb-2 block text-xs font-black tracking-wider text-[#15803d]">
              {currentQ.character}
            </span>
          ...
          <div className="flex h-56 w-56 animate-pulse items-center justify-center rounded-full border-4 border-black bg-[#111827] text-7xl shadow-[0px_0px_35px_rgba(34,197,94,0.3)]">
            {currentQ.icon}
          </div>
        ...
        <div className="flex min-h-[450px] w-full max-w-xl flex-col justify-between rounded-2xl border-4 border-black bg-[#fffbeb] p-6 text-black shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] lg:w-1/2">
          ...
          <div className="mt-6 flex justify-end border-t-2 border-dashed border-gray-300 pt-4">
            ...
            >
              {currentStage === informaticsGameLevels.length - 1
                ? "ЖЫЙЫНТЫКТЫ КӨРҮҮ 🏆"
                : "КИЙИНКИ СУРОО ➡️"}
            </button>
          </div>
...
export default InformaticsGame;
