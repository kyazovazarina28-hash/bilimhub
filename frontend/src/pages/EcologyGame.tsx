...
import { ecologyGameLevels } from "../data/ecologyGameLevels";

interface EcologyComment {
  id: number;
  user: string;
  text: string;
}

const SEED_COMMENTS: Record<number, EcologyComment[]> = {
  1: [
    { id: 1, user: "Айдар", text: "Пластикти колдонбогонго аракет кылышыбыз керек! 🌲" },
    { id: 2, user: "Каныкей", text: "Пластик туура табылды! ♻️" },
  ],
  2: [
    { id: 1, user: "Айдар", text: "Пластикти колдонбогонго аракет кылышыбыз керек! 🌲" },
    { id: 2, user: "Каныкей", text: "Бул сабак баарына керек экен, абдан маанилүү тема ✨" },
  ],
};

const DEFAULT_LIKES: Record<number, number> = { 1: 389, 2: 342 };
const DEFAULT_SHARES: Record<number, number> = { 1: 38, 2: 31 };
...
const defaultLikes = (id: number) => DEFAULT_LIKES[id] ?? 48 + ((id * 6) % 75);
const defaultShares = (id: number) => DEFAULT_SHARES[id] ?? 11 + (id % 20);

const EcologyGame: FC = () => {
  ...
  const [likes, setLikes] = useState<Record<number, number>>(() =>
    loadJson("ecology_likes", DEFAULT_LIKES),
  );
  const [isLiked, setIsLiked] = useState<Record<number, boolean>>(() =>
    loadJson("ecology_is_liked", {}),
  );
  const [shares, setShares] = useState<Record<number, number>>(() =>
    loadJson("ecology_shares", DEFAULT_SHARES),
  );
  const [comments, setComments] = useState<Record<number, EcologyComment[]>>(() =>
    loadJson("ecology_comments", SEED_COMMENTS),
  );
  ...
  const currentQ = ecologyGameLevels[currentStage] ?? ecologyGameLevels[0];
  ...
  useEffect(() => {
    localStorage.setItem("ecology_likes", JSON.stringify(likes));
  }, [likes]);

  useEffect(() => {
    localStorage.setItem("ecology_is_liked", JSON.stringify(isLiked));
  }, [isLiked]);

  useEffect(() => {
    localStorage.setItem("ecology_shares", JSON.stringify(shares));
  }, [shares]);

  useEffect(() => {
    localStorage.setItem("ecology_comments", JSON.stringify(comments));
  }, [comments]);
  ...
  const getRank = () => {
    if (score.points >= 600) return "👑 ECO GUARDIAN";
    if (score.points >= 300) return "🌲 ТАБИЯТ КОРГООЧУСУ";
    if (score.points >= 100) return "♻️ ЭКОЛОГ";
  ...
  const handleNext = () => {
    if (selectedOption === null) return;
    if (currentStage < ecologyGameLevels.length - 1) {
  ...
  const shareToSocial = (
    ...
    const text = encodeURIComponent(
      `Экологиялык миссия оюнун ойноп жатам! 🌍 ${window.location.href}`,
    );
  ...
  if (isFinished) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#052e16] p-4 font-sans text-white">
        <div className="w-full max-w-md rounded-2xl border-4 border-black bg-[#fffbeb] p-8 text-center text-black shadow-[8px_8px_0px_#000]">
          ...
          <div className="mb-6 rounded-xl border-4 border-black bg-black p-4 text-white">
            <p className="mb-2 text-sm font-black text-[#86efac]">{getRank()}</p>
          ...
          <div className="space-y-3">
            <button
              type="button"
              onClick={restartGame}
              className="w-full rounded-xl border-4 border-black bg-[#22c55e] py-3 font-black text-black shadow-[3px_3px_0px_#000]"
            >
              🔄 КАЙРА БАШТОО
            </button>
            <button
              type="button"
              onClick={() => navigate("/subjects/ekologiya")}
  ...
  return (
    <div className="relative flex min-h-screen flex-col justify-between overflow-hidden bg-[#052e16] p-4 font-sans text-white">
      <div
        ref={trailRef}
      ...
      {shareSuccess && (
        <div className="fixed left-1/2 top-6 z-50 -translate-x-1/2 animate-bounce rounded-xl border-4 border-black bg-[#86efac] px-6 py-3 text-xs font-black uppercase tracking-wider text-black shadow-[4px_4px_0px_#000]">
      ...
      <div className="z-10 mx-auto mt-2 flex w-full max-w-7xl items-center justify-between">
        ...
        <div className="text-center">
          <h1 className="text-xl font-black tracking-wider text-[#86efac] md:text-2xl">
            ECOLOGY QUEST 🌱
          </h1>
          <p className="mt-1 text-xs font-bold uppercase tracking-widest text-emerald-300">
            МИССИЯ: {currentStage + 1} / {ecologyGameLevels.length}
          </p>
        </div>
        <div className="rounded-xl border-2 border-black bg-[#22c55e] px-4 py-2 font-black text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
      ...
      <div className="z-10 mx-auto my-auto flex w-full max-w-7xl flex-col items-center justify-center gap-8 px-4 lg:flex-row">
        <div className="flex w-full max-w-lg flex-col items-center lg:w-1/2">
          ...
          <div className="flex h-56 w-56 animate-pulse items-center justify-center rounded-full border-4 border-black bg-[#14532d] text-7xl shadow-[0px_0px_35px_rgba(34,197,94,0.3)]">
            {currentQ.icon}
          </div>
        ...
        <div className="flex min-h-[450px] w-full max-w-xl flex-col justify-between rounded-2xl border-4 border-black bg-[#fffbeb] p-6 text-black shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] lg:w-1/2">
          <div>
            <div className="mb-4 inline-block rounded-md border-2 border-black bg-[#dcfce7] px-3 py-1 text-xs font-black uppercase tracking-wider text-[#15803d]">
              {currentQ.tag}
            </div>
          ...
          <div className="mt-6 flex justify-end border-t-2 border-dashed border-gray-300 pt-4">
            <button
              ...
              className={`flex items-center gap-2 rounded-xl border-2 border-black px-6 py-3 font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all ${
                selectedOption !== null
                  ? "bg-[#22c55e] text-white hover:translate-y-0.5"
                  : "cursor-not-allowed bg-gray-300 text-gray-500 shadow-none"
              }`}
            >
              {currentStage === ecologyGameLevels.length - 1
                ? "ЖЫЙЫНТЫКТЫ КӨРҮҮ 🏆"
                : "КИЙИНКИ СУРОО ➡️"}
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border-4 border-black bg-[#fffbeb] p-6 text-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <div className="mb-4 flex items-center justify-between border-b-2 border-black pb-2">
              <h3 className="text-lg font-black text-[#15803d]">💬 ПИКИРЛЕР</h3>
              <button
                type="button"
            ...
            <div className="mb-4 max-h-48 space-y-2 overflow-y-auto">
              {levelComments.map((c) => (
                ...
                >
                  <span className="block font-bold text-green-700">{c.user}:</span>
            ...
            <form onSubmit={handleAddComment} className="flex gap-2">
              <input
                ...
                placeholder="Экологиялык пикир..."
                className="w-full rounded-xl border-2 border-black p-2 text-sm"
              />
              <button
                type="submit"
                className="rounded-xl border-2 border-black bg-[#22c55e] px-4 text-xs font-black text-white"
              >
                ЖӨНӨТҮҮ
              </button>
            </form>
      ...
      {isShareOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border-4 border-black bg-[#fffbeb] p-6 text-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <div className="mb-4 flex items-center justify-between border-b-2 border-black pb-2">
              <h3 className="text-lg font-black text-[#15803d]">🔁 БӨЛҮШҮҮ</h3>
              <button
                type="button"
      ...
      <div className="z-10 mx-auto mt-4 flex w-full max-w-7xl flex-col items-center justify-between gap-2 rounded-xl border-t-4 border-black bg-[#022c15] p-4 sm:flex-row">
        ...
        <div className="text-xs font-black uppercase tracking-widest text-[#86efac]">
          ЭКО ДЕҢГЭЭЛ: {getRank()}
        </div>
