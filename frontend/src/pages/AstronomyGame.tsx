...
import { astronomyGameLevels } from "../data/astronomyGameLevels";

interface AstronomyComment {
  id: number;
  user: string;
  text: string;
}

const SEED_COMMENTS: Record<number, AstronomyComment[]> = {
  1: [
    { id: 1, user: "Тимур", text: "Марс тууралуу дагы суроолорду кошугулачы! 🔴" },
    { id: 2, user: "Айсулуу", text: "Күн туура табылды! ☀️" },
  ],
  2: [
    { id: 1, user: "Тимур", text: "Марс тууралуу дагы суроолорду кошугулачы! 🔴" },
    { id: 2, user: "Айсулуу", text: "Космос темасы абдан сырдуу жана кызыктуу экен 🌌" },
  ],
};

const DEFAULT_LIKES: Record<number, number> = { 1: 420, 2: 385 };
const DEFAULT_SHARES: Record<number, number> = { 1: 42, 2: 36 };
...
const defaultLikes = (id: number) => DEFAULT_LIKES[id] ?? 50 + ((id * 7) % 80);
const defaultShares = (id: number) => DEFAULT_SHARES[id] ?? 12 + (id % 22);

const AstronomyGame: FC = () => {
  ...
  const [likes, setLikes] = useState<Record<number, number>>(() =>
    loadJson("astronomy_likes", DEFAULT_LIKES),
  );
  const [isLiked, setIsLiked] = useState<Record<number, boolean>>(() =>
    loadJson("astronomy_is_liked", {}),
  );
  const [shares, setShares] = useState<Record<number, number>>(() =>
    loadJson("astronomy_shares", DEFAULT_SHARES),
  );
  const [comments, setComments] = useState<Record<number, AstronomyComment[]>>(() =>
    loadJson("astronomy_comments", SEED_COMMENTS),
  );
  ...
  const currentQ = astronomyGameLevels[currentStage] ?? astronomyGameLevels[0];
  ...
  useEffect(() => {
    localStorage.setItem("astronomy_likes", JSON.stringify(likes));
  }, [likes]);

  useEffect(() => {
    localStorage.setItem("astronomy_is_liked", JSON.stringify(isLiked));
  }, [isLiked]);

  useEffect(() => {
    localStorage.setItem("astronomy_shares", JSON.stringify(shares));
  }, [shares]);

  useEffect(() => {
    localStorage.setItem("astronomy_comments", JSON.stringify(comments));
  }, [comments]);
  ...
  const getRank = () => {
    if (score.points >= 600) return "👑 SPACE EXPLORER";
    if (score.points >= 300) return "🚀 КОСМОНАВТ";
    if (score.points >= 100) return "🔭 АСТРОНОМ";
  ...
  const handleNext = () => {
    if (selectedOption === null) return;
    if (currentStage < astronomyGameLevels.length - 1) {
  ...
  const shareToSocial = (
    ...
    const text = encodeURIComponent(
      `Космостук саякат оюнун ойноп жатам! 🚀 ${window.location.href}`,
    );
  ...
  if (isFinished) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#09071a] p-4 font-sans text-white">
        <div className="w-full max-w-md rounded-2xl border-4 border-black bg-[#fffbeb] p-8 text-center text-black shadow-[8px_8px_0px_#000]">
          ...
          <div className="mb-6 rounded-xl border-4 border-black bg-black p-4 text-white">
            <p className="mb-2 text-sm font-black text-[#f59e0b]">{getRank()}</p>
            <div className="mt-3 grid grid-cols-2 gap-3 text-center text-xs">
              ...
              <div>
                ...
                <p className="text-base font-black text-blue-400">{score.crystals}</p>
              </div>
              <div>
          ...
          <div className="space-y-3">
            <button
              type="button"
              onClick={restartGame}
              className="w-full rounded-xl border-4 border-black bg-[#3b82f6] py-3 font-black text-white shadow-[3px_3px_0px_#000]"
            >
              🔄 КАЙРА БАШТОО
            </button>
            <button
              type="button"
              onClick={() => navigate("/subjects/astronomiya")}
  ...
  return (
    <div className="relative flex min-h-screen flex-col justify-between overflow-hidden bg-[#09071a] p-4 font-sans text-white">
      <div
        ref={trailRef}
      ...
      {shareSuccess && (
        <div className="fixed left-1/2 top-6 z-50 -translate-x-1/2 animate-bounce rounded-xl border-4 border-black bg-[#f59e0b] px-6 py-3 text-xs font-black uppercase tracking-wider text-black shadow-[4px_4px_0px_#000]">
      ...
      <div className="z-10 mx-auto mt-2 flex w-full max-w-7xl items-center justify-between">
        ...
        <div className="text-center">
          <h1 className="text-xl font-black tracking-wider text-[#f59e0b] md:text-2xl">
            AALAM QUEST 🪐
          </h1>
          <p className="mt-1 text-xs font-bold uppercase tracking-widest text-blue-400">
            ОРБИТА: {currentStage + 1} / {astronomyGameLevels.length}
          </p>
        </div>
        <div className="rounded-xl border-2 border-black bg-[#3b82f6] px-4 py-2 font-black text-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
      ...
      <div className="z-10 mx-auto my-auto flex w-full max-w-7xl flex-col items-center justify-center gap-8 px-4 lg:flex-row">
        <div className="flex w-full max-w-lg flex-col items-center lg:w-1/2">
          <div className="relative mb-8 w-full rounded-2xl border-4 border-black bg-white p-6 text-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <span className="mb-2 block text-xs font-black tracking-wider text-[#1e40af]">
              {currentQ.character}
            </span>
          ...
          <div className="flex h-56 w-56 animate-pulse items-center justify-center rounded-full border-4 border-black bg-[#130f30] text-7xl shadow-[0px_0px_35px_rgba(59,130,246,0.3)]">
            {currentQ.icon}
          </div>
        ...
        <div className="flex min-h-[450px] w-full max-w-xl flex-col justify-between rounded-2xl border-4 border-black bg-[#fffbeb] p-6 text-black shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] lg:w-1/2">
          <div>
            <div className="mb-4 inline-block rounded-md border-2 border-black bg-[#dbeafe] px-3 py-1 text-xs font-black uppercase tracking-wider text-[#1e40af]">
              {currentQ.tag}
            </div>
          ...
          <div className="mt-6 flex justify-end border-t-2 border-dashed border-gray-300 pt-4">
            <button
              ...
              className={`flex items-center gap-2 rounded-xl border-2 border-black px-6 py-3 font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all ${
                selectedOption !== null
                  ? "bg-[#3b82f6] text-white hover:translate-y-0.5"
                  : "cursor-not-allowed bg-gray-300 text-gray-500 shadow-none"
              }`}
            >
              {currentStage === astronomyGameLevels.length - 1
                ? "ЖЫЙЫНТЫКТЫ КӨРҮҮ 🏆"
                : "КИЙИНКИ СУРОО ➡️"}
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border-4 border-black bg-[#fffbeb] p-6 text-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <div className="mb-4 flex items-center justify-between border-b-2 border-black pb-2">
              <h3 className="text-lg font-black text-[#1e40af]">💬 ПИКИРЛЕР</h3>
              <button
                type="button"
            ...
            <form onSubmit={handleAddComment} className="flex gap-2">
              <input
                ...
                placeholder="Космостук пикир..."
                className="w-full rounded-xl border-2 border-black p-2 text-sm"
              />
              <button
                type="submit"
                className="rounded-xl border-2 border-black bg-[#3b82f6] px-4 text-xs font-black text-white"
              >
                ЖӨНӨТҮҮ
              </button>
            </form>
      ...
      {isShareOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border-4 border-black bg-[#fffbeb] p-6 text-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <div className="mb-4 flex items-center justify-between border-b-2 border-black pb-2">
              <h3 className="text-lg font-black text-[#1e40af]">🔁 БӨЛҮШҮҮ</h3>
              <button
                type="button"
      ...
      <div className="z-10 mx-auto mt-4 flex w-full max-w-7xl flex-col items-center justify-between gap-2 rounded-xl border-t-4 border-black bg-[#02020a] p-4 sm:flex-row">
